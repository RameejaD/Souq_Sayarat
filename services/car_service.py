...
class CarService:
    ...
    def create_car(self, user_id, ad_title, description, car_information, exterior_color, trim, regional_specs,
                  ...
                  draft=False, consumption='0', no_of_cylinders='0', interior='', payment_option='', skip_required_check=False):
        """Create a new car listing"""
        # Only validate required fields if not a draft and not skipping required check
        if not draft and not skip_required_check:
            required_fields = {
                'user_id': user_id,
                'ad_title': ad_title,
    ...
    def get_car_by_id_delete(self, car_id):
        """Get a car by ID"""
        car = self.car_repository.get_car_by_id(car_id)
        return car
